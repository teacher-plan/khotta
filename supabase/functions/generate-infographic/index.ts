...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userPrompt = [
      ...
      bookContext
        ? `نص الدرس من كتاب الطالب المعتمد — التزم بمحتواه الفعلي حرفياً (لا تُلخّص الفكرة العامة وتبني محتوى جديداً من عندك، ولا تُعمِّم أو تفترض محتوى غير موجود فيه):\n${bookContext}`
        : bookImages.length
        ? "الصور المرفقة هي صفحات هذا الدرس من كتاب الطالب المعتمد — افحصها بصرياً بدقّة، اقرأ كل نصٍّ ورقمٍ ورسمٍ فيها فعلياً، والتزم بمحتواها الفعلي الحرفي (لا تُلخّص الفكرة العامة وتبني محتوى جديداً من عندك، ولا تكتفِ بتخمين محتواها من عنوان الدرس)."
