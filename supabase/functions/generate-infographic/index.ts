...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userPrompt = [
      ...
      "السياق التعليمي المحدد: منهج كامبردج (Cambridge) المعتمد في مدارس سلطنة عُمان — سلطنة عُمان تحديداً لا أي دولة أخرى.",
      age ? `أعمار الطلاب: ${age} سنوات تقريباً — المحتوى واللغة والرسومات تناسب هذا العمر بدقة.` : "",
      ...
      bookContext
        ? `ملخص فعلي لمحتوى هذا الدرس من كتاب الطالب المعتمد — لخّص الإنفوجرافيك منه حصراً (لا من معرفة عامة):\n${bookContext}`
        : bookImages.length
        ? "الصور المرفقة هي صفحات هذا الدرس من كتاب الطالب المعتمد — اقرأ محتواها ولخّص مفاهيمه وأمثلته وقواعده الفعلية في الإنفوجرافيك (لا من معرفة عامة)."
        : "لا صفحات كتاب مرفقة — بناءً على خبرتك بمنهج كامبردج المعتمد في سلطنة عُمان لهذا الصف والمادة، توقّع المحتوى الفعلي المرجّح لهذا الدرس تحديداً (لا محتوى عام) ولخّصه بثقة.",
