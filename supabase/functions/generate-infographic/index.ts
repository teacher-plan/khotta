...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userPrompt = [
      ...
      bookContext
        ? `نص الدرس من كتاب الطالب المعتمد (مرجعٌ لاستخراج الأفكار الرئيسية بنفس تسلسله — ليس نصاً يُنسخ حرفياً):\n${bookContext}`
        : bookImages.length
        ? "الصور المرفقة هي صفحات هذا الدرس من كتاب الطالب المعتمد — مرجعٌ لاستخراج الأفكار الرئيسية فقط، لا لنسخ نصّها حرفياً."
        : "لا صفحات كتاب مرفقة — بناءً على خبرتك بمنهج كامبردج المعتمد في سلطنة عُمان لهذا الصف والمادة، توقّع المحتوى الفعلي المرجّح لهذا الدرس تحديداً (لا محتوى عام) واستخرج أفكاره الرئيسية.",
      "لخّص نص الدرس لاستخراج أفكاره الرئيسية فقط، ثم رتّب هذه الأفكار في الإنفوجرافيك بنفس تسلسل الكتاب (أولاً، ثانياً، ثالثاً...) — لا تُعِد ترتيب المفاهيم بمنطقٍ خاصٍّ بك.",
      "ممنوعٌ نسخ أي نصٍّ حرفي من الدرس داخل الإنفوجرافيك، وبالأخصّ نص أي سؤال تمرين أو تقويم — الإنفوجرافيك ملخّصٌ للمفاهيم لا نسخةٌ من نص الكتاب أو تمارينه.",
      "كل الأمثلة داخل الإنفوجرافيك يجب أن تكون من إبداعك الخاص (لا اقتباساً من نص الكتاب أو أسئلته)، بحيث تناسب محتوى الدرس وعنوانه والفئة العمرية والهوية العُمانية.",
      "رتّب أهم المفاهيم والقواعد في أقسامٍ واضحة مرقّمة (أولاً، ثانياً، ثالثاً...) بنفس تسلسل الكتاب، مع تمثيل بصري مرسوم لكل مفهوم.",
