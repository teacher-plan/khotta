...
// النصف النصّي من العرض التقديمي: يقرأ الدرس مرّةً واحدة ويُخرج لكل شريحة
// عنواناً ونقاطاً مبنيّةً على محتوى الكتاب الفعلي، + وصفاً بصرياً
// (imagePrompt) يُكمّل به نموذجُ الصور رسمَ الشريحة.
//
// لماذا يبقى هذا النداء رغم أن نموذج الصور يرسم الشريحة كاملةً بنصّها:
// لأن نموذج الصور لا يقرأ صفحات الكتاب ولا يبني تسلسلاً تربوياً؛ يرسم ما
// يُملى عليه. هذه الدالة هي مصدر المحتوى المطابق للمنهج، وتلك أداةُ إخراجه.
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const system = [
      ...
      "",
      "حقل imagePrompt — وصفٌ عربيٌّ موجز (جملة إلى جملتين) للتمثيل البصري التعليمي الذي يوضّح محتوى هذه الشريحة: أشياء واقعية مألوفة للطالب من بيئته، ومخططٌ يشرح المفهوم لا زخرفةٌ حوله. نموذج الصور هو من سيرسم الشريحة كاملةً بعنوانها ونقاطها، فاجعل الوصف مكمّلاً لها لا بديلاً عنها.",
