...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const schema =
      '{"units":[{"unit":"اسم الوحدة","lessons":["اسم الدرس ١","اسم الدرس ٢"]}]}';

    const system = [
      "أنت مساعد يستخرج فهرس محتويات كتاب مدرسي عُماني من صورة.",
      "استخرج أسماء الوحدات وأسماء الدروس تحت كل وحدة بالترتيب كما تظهر تماماً.",
      "لا تختلق أي درس غير موجود في الصورة. تجاهل أرقام الصفحات والعناوين العامة (مقدمة/فهرس).",
      "إن لم توجد وحدات صريحة، اجعل الدروس تحت وحدة باسم «عام».",
      "أعد JSON فقط دون أي نص خارجه.",
    ].join("\n");

    const content: unknown[] = [
      { type: "text", text: `استخرج الوحدات والدروس بهذا الشكل حصراً (JSON): ${schema}` },
    ];
