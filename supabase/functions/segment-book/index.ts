...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const model = ensureVision(st.model_segment || st.seg_model || st.vision_model || "google/gemini-2.5-flash", "google/gemini-2.5-flash");
    ...
    if (!orResp.ok) {
      ...
      console.error(`openrouter ${orResp.status} في segment-book: ${_m}`);
