...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── إعدادات طبيعية: أُلغيت كل التعليمات التفصيلية (منع النص، منع
    // الوجوه، الألوان، حدّ الحجم...)، وأُبقي فقط على ما لا غنى عنه: الفئة
    // العمرية والهوية العُمانية. البقية تُترك لاجتهاد النموذج الافتراضي.
    const gradeNum = parseInt(grade) || 0;
    const age = gradeNum ? gradeNum + 6 : 0;
    const style = st.slide_style_prompt || [
      age ? `الفئة العمرية: أطفال ${age} سنوات تقريباً (الصف ${grade}) — راعِ ذلك في المحتوى والأسلوب.` : "",
      "الهوية العُمانية حصراً إن ظهرت أزياء أو مبانٍ أو شخصيات (الزي المدرسي العُماني، الكمة والدشداشة) — لا مظاهر أي دولة خليجية أخرى.",
    ].filter(Boolean).join(" ");

    const kindPrompt = kind === "cover"
      ? "هذه شريحة الغلاف الافتتاحية."
      : kind === "closing"
        ? "هذه الشريحة الختامية."
        : "هذه شريحة محتوى ضمن العرض.";
    ...
    const userPrompt = reserveMode
      ...
      : [
        `ارسم شريحة عرض تقديمي تعليمية واحدة (الشريحة ${slideNo} من ${slideTotal}) لدرس «${safeTitle}» في مادة ${subject}${grade ? " للصف " + grade : ""}.`,
        kindPrompt,
        bullets.length ? `أهم أفكار الدرس (من محتوى الكتاب):\n${bullets.map((x) => `- ${x}`).join("\n")}` : "",
        style,
      ].filter(Boolean).join("\n");
