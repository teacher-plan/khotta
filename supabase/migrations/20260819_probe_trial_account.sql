...
BEGIN
  ...
  SELECT format(
    'uid=%s | allowed=%s | c1prof_updated=%s | banned=%s | gen_enabled=%s | ai_month=%s | usage=%s | last_err=%s | last_cost=%s',
    uid::text,
    ...
    (SELECT format('%s | %s | %s',occurred_at,function_name,message) FROM public.error_logs WHERE user_id=uid ORDER BY occurred_at DESC LIMIT 1),
    (SELECT format('%s | %s | %s | ok=%s',created_at,function_name,model,success) FROM public.ai_cost_log WHERE user_id=uid ORDER BY created_at DESC LIMIT 1)
  ) INTO v;
