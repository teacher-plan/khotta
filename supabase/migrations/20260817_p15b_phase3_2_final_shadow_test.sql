-- ════════════════════════════════════════════════════════════════════
-- Phase 3.2 FINAL SHADOW VALIDATION — بعد انتظار نافذة cooldown كاملةً
-- (30+ دقيقة) بلا أي نشرٍ جديد أو تعديل Playbook/Action Registry/شروط
-- أمان. حادثةٌ اصطناعية آمنة موسومة، محفوظةٌ فعلياً ثم مُنظَّفة.
-- ════════════════════════════════════════════════════════════════════
...
INSERT INTO public.ops_incidents (
  ...
  '__phase3_2_final_test__credit-monitor',
  'credit-monitor', 'P2', 'DETECTED', 'VERIFIED',
  '[اختبارٌ آمن Phase 3.2 FINAL] فشلٌ متكرّرٌ اصطناعي بعد انتظار cooldown كاملٍ',
  jsonb_build_array(jsonb_build_object('source','phase3_2_final_test','detail','3 فشلاتٍ اصطناعية بنفس رسالة الخطأ: TimeoutError')),
...
DECLARE
  ...
  v_escalation text;
BEGIN
  SELECT id INTO v_incident_id FROM public.ops_incidents WHERE dedup_key = '__phase3_2_final_test__credit-monitor';
  ...
  IF v_last_deploy IS NULL THEN
    ...
    v_deploy_detail := 'لا سجلّ نشرٍ متاحٌ بعد — NOT_AVAILABLE.';
  ELSE
    ...
    v_deploy_detail := format('آخر نشرٍ كان منذ %s دقيقة (cooldown=30) — %s', round(v_minutes_since_deploy,1),
      CASE WHEN v_deploy_passed THEN 'أقدم من النافذة، السياق مستقرّ.' ELSE 'لا يزال ضمن النافذة.' END);
  END IF;
  ...
  v_preconditions := jsonb_build_array(
    ...
    jsonb_build_object('name','no_similar_incident_open_recently','passed', v_similar_count <= 1, 'detail', v_similar_count || ' حادثةٍ غير محلولة لنفس المكوّن آخر 30 دقيقة.'),
  ...
  ELSE
    -- Shadow Mode فقط: تُسجَّل النتيجة WOULD_AUTO_HEAL، بلا أي تنفيذٍ فعلي إطلاقاً.
  ...
  INSERT INTO public.repair_executions (
    ...
    1,
    jsonb_build_object('consecutive_failures', 3, 'distinct_errors', jsonb_build_array('TimeoutError'), 'agent_id', 'credit-monitor', 'test_scenario', true, 'phase', '3.2-final'),
