...
> **المصدر**: هذه المهارة من إعداد [Emil Kowalski](https://github.com/emilkowalski/skills)
> (صانع مكتبتَي Sonner وVaul)، مستخلَصةٌ من محاضرات Apple في WWDC.
> أُضيفت إلى مشروع خُطّتي كمرجعٍ لتحسين الواجهات.
> **انظر قسم «ملاحظات خاصة بمنصّة خُطّتي» في نهاية الملف — فيه تحفّظٌ مهمٌّ
> يخصّ النصوص العربية يجب الالتزام به.**
...
# ⚠️ ملاحظات خاصة بمنصّة خُطّتي (إضافةٌ محلّية، ليست من المصدر الأصلي)

## ١. التتبّع (letter-spacing) محظورٌ في النصوص العربية

القسم ١٥ أعلاه يوصي بـ`letter-spacing` سالبٍ للعناوين وموجبٍ للنصوص الصغيرة.
**هذه النصيحة للخط اللاتيني حصراً.** في العربية، الحروف **متّصلة**، وأي
`letter-spacing` يكسر الاتصال ويُشوّه شكل الكلمة ويُضعف القراءة.

**القاعدة في هذا المشروع:**
- ❌ لا `letter-spacing` على أي نصٍّ عربي إطلاقاً (`letter-spacing: 0` أو تركه بلا تعريف).
- ✅ يُسمح به فقط على نصوصٍ لاتينية بحتة (أكواد، أرقام إنجليزية، تسميات تقنية).
- ✅ بقية نصائح القسم ١٥ (الوزن، `line-height`، `rem` بدل `px`) صالحةٌ للعربية.

ملاحظة إضافية: العربية تحتاج **`line-height` أكبر** من اللاتينية عادةً
(١٫٧–١٫٩ للنصّ الجاري) بسبب التشكيل والنقاط تحت السطر وفوقه.

## ٢. الاتجاه: RTL

كل الواجهات `dir="rtl"`. عند تطبيق القسم ٧ (الاتساق المكاني):
- ما يدخل من **اليمين** يجب أن يخرج من **اليمين** — والاتجاه في RTL معكوسٌ
  عن أمثلة المصدر.
- استخدم الخصائص المنطقية (`inset-inline-start`, `margin-inline`) بدل
  `left`/`right` الصريحة كي لا ينكسر الاتجاه.

## ٣. لا مكتبات خارجية

المشروع صفحاتٌ مستقلّة بلا نظام بناء (no build step). لذلك:
- ❌ لا `import` لـMotion/Framer Motion كما في أمثلة القسم ٤.
- ✅ المبادئ نفسها (قابلية المقاطعة، تسليم السرعة، إسقاط الزخم) تُنفَّذ
  بجافاسكربت خالصة عبر `requestAnimationFrame` — الصيغ الرياضية في
  القسمين ٥ و٦ مستقلّةٌ عن أي مكتبة وتعمل كما هي.

## ٤. أداء الأجهزة

المعلّمات يستخدمن أجهزةً متفاوتة. `backdrop-filter` (القسم ١٢) مكلفٌ على
الأجهزة الضعيفة — استخدمه على أسطحٍ صغيرةٍ ومحدودة (شريط علوي، ورقة سفلية)،
لا على مساحاتٍ كبيرةٍ أو عناصر متكرّرة في قائمة.
